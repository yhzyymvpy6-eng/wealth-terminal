...
export default function MonteCarloProjections() {
  ...
  return (
    <div className="glass p-8 rounded-3xl">
      ...
      <p className="text-gray-400 mb-8">Monte Carlo Simulation — 1,000 scenarios</p>

      <div className="mb-8">
        <label className="text-sm text-gray-400 block mb-3">Monthly Contribution: £{monthlyContribution}</label>
        <input 
          type="range" 
          min="0" 
          max="2000" 
      ...
      <div className="flex gap-3 mb-8 overflow-x-auto pb-2">
        {[10, 20, 30].map(y => (
          <button 
            ...
            className={`px-6 py-3 rounded-2xl whitespace-nowrap text-sm font-medium ${selectedYear === y ? 'bg-white text-black' : 'bg-gray-900'}`}
          >
            {y} Years
          </button>
        ))}
